...
const stan = {
    ...
    tokenBase(stream, state) {
        ...
        if (stream.match('/*')) {
            state.tokenize = this.tokenComment;
            return 'comment';
        }
        ...
        if (stream.match('#')) {
            ...
            if (stream.match(/\s*include\b/)) {
                state.tokenize = this.tokenInclude;
                return 'meta';
            }
            stream.skipToEnd();
            return 'comment';
        ...
        // Strings
        if (stream.match('"')) {
            state.tokenize = this.tokenString;
            return 'string';
        }
